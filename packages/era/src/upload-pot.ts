...
// Produces `eras-<range>.pot.json` with the root references
// (byNumber / byHash / byTx / byAddress / byBalanceBlock / meta) plus upload
// stats. Balance-event uploads can be skipped with --no-state.
//
// Usage:
...
//   pnpm era:upload-pot --batch-id abc123... --no-state 0..6
...
async function loadExistingRefs(path: string): Promise<PotIndexRefs> {
  ...
  return {
    ...
    byAddress: meta.indexes.byAddress ?? null,
    byBalanceBlock: meta.indexes.byBalanceBlock ?? null,
...
if (!args.batchId) {
  ...
  console.error(
    '  pnpm era:upload-pot --batch-id abc123... --no-state 0..6   # skip balance-events upload',
  )
  process.exit(1)
}
...
console.log(`       byAddress:      ${indexRefs.byAddress ?? '(none)'}`)
console.log(`       byBalanceBlock: ${indexRefs.byBalanceBlock ?? '(none)'}`)
...
// Upload an envelope JSON carrying every POT ref; the feed stores only
